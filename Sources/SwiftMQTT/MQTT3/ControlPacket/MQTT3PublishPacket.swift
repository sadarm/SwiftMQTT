...
extension MQTT3 {
    struct PublishPacket: MQTT3ControlPacket {
        struct Flags {
            var dupFlag: Bool
            var qos: MQTTQoS
            var retain: Bool
            ...
            init?(byte: UInt8) {
                guard let qos = MQTTQoS(rawValue: (byte & 0x06) >> 1) else {
                    return nil
                }
        ...
        var typeAndFlags: ControlPacketTypeAndFlags {
            ControlPacketTypeAndFlags(type: .publish, flags: self.dupFlag.byte << 3 | self.qosWithIdentifier.qos.rawValue << 1 | self.retain.byte)
        }
...
extension MQTT3.PublishPacket {
    init(typeAndFlags: MQTT3.ControlPacketTypeAndFlags, data: Data) throws {
        guard typeAndFlags.type == .publish else {
            throw SwiftMQTTError.typeMissmatch
        }
        
        guard let flags = MQTT3.PublishPacket.Flags(byte: typeAndFlags.rawValue) else {
